...
**W2 — Executor breadth (2026-07-22).** Generic CLI adapter: any coding agent is a config row (command template, parser, timeout), not code. opencode/pi/aider seeded; `code-local` task type; per-task executor+model override; Settings panel. Also hardened honesty: an executor that prints nothing, or changes no files, is now reported as failed rather than done.
