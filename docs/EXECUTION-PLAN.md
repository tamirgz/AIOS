...
*2026-07-20. The single consolidated sequence. Details live in WORKBENCH-PLAN.md (Workbench design/research) and ONE-STOP-PLAN.md (integrations, token policy, distillation strategy); this file is the order of battle.*
...
| # | Phase | Contents | Exit test | Your setup |
|---|---|---|---|---|
| 0 | **Activation** *(anytime, ~15 min total)* | GCP OAuth client → Settings; Slack bot token + channels; Slack webhook | Calendar shows your event colors; #tldr/#my-today appear in External Reports; bell notifications reach Slack | All of it |
| 1 | **W1 — Workbench spine** | Task/attempt/event schema; supervision engine (worktrees, timeouts, SIGTERM-safe, reconciliation); claude-headless + native adapters; cards + drawer UI (live tail, diff) | Delegate "research X" and "fix this bug" from one text box; both finish unattended; diff reviewed in-app | — |
| 2 | **W2 — Executor breadth** | opencode-serve sidecar (SSE + approvals bridge); generic CLI adapter, pi + aider seeded; task-type defaults in Settings; retry-as-attempt | Same coding task run on Claude and opencode+qwen as sibling attempts, compared by diff | opencode installed (`brew/npm`) |
...
| 4 | **Gmail + local rerouting** *(ONE-STOP P1)* | Gmail read-only module (same OAuth); brief v2 (calendar+tasks+email+agent findings); reroute triage/light-enrichment/digests to Ollama | One morning where the brief made opening Gmail/Calendar unnecessary; >80 % of AI calls local (usage panel) | `gmail.readonly` scope on the OAuth consent screen |
| 5 | **"Ask" + Notion** *(P2)* | Notion read-only index; Ask page — cited Q&A over knowledge/vault/notion/gmail, Ollama-first with escalate button; Gmail history embedded | 3 NotebookLM-grade questions answered with citations, zero Claude tokens | Notion integration token |
| 6 | **Reach & act** *(P3)* | Telegram bridge (capture/brief/approvals from phone); approval-gated Gmail drafts; Calendar write-back | From the phone: capture, receive brief, approve one real action | Tailscale (or tunnel) |
