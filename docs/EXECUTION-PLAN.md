...
**W2 — Executor breadth (2026-07-22, exit test passed 2026-07-23).** Generic CLI adapter: any coding agent is a config row (command template, parser, timeout), not code. opencode/pi/aider seeded; `code-local` task type; per-task executor+model override; Settings panel. Honesty hardened: an executor that prints nothing, or changes no files, is reported as failed, not done. CLI executors run in a local **clone** (real `.git`, correct project root), and `PWD` is pinned to the isolated dir — without which opencode edited the AIOS checkout itself. Exit test passes: Claude and free local opencode+qwen each produce a correct diff on one card.
