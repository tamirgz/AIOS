...
| 3 | **L1 — Attention spine** *(next; absorbs old W3)* | `attention_items` atom; **"Needs you" queue** (aggregates items + approvals + Workbench `needs_input`); `nextAction` on projects; **Plan-my-day** surface (→ writes calendar blocks); **Daily planner** agent on `qwen3:8b`; trust gradient enforced | A week of mornings where a 30-sec approve set the day, and every "needs you" moment lived in one queue | ⬜ **not started.** Design in ONE-STOP §3. Small W2 tails ride along: pi parser vs. a real fixture; install/verify aider; Workbench merge/PR button |
