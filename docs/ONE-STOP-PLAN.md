...
| Gmail | ❌ Not built (after L1–L3) — the OAuth client is already there, it needs the `gmail.readonly` scope; feeds the follow-up agent |
| Notion | ❌ Not built (later — the "Ask" phase) |
...
- **People/follow-ups** (CRM-lite): a light `people` table tied to calendar attendees; **Follow-up tracker** agent → `approve`-draft / `do` items after meetings.
- **Loose-ends chaser** + **Connector** agents (proactive relations). **Weekly reviewer** (`gemma4:31b-it-qat`) → Friday synthesis + proposed next week.
- Exit: a week where AIOS chased the loose ends and prepped the follow-ups before you noticed them.
