  ...
  agentTemplates: [
    {
      ...
      defaultPrompt: [
        ...
        "3. If the focused project has no goal, set one with projects.setGoal (targets the focused project — no id). For the next-action: do NOT set one when the project has OPEN tasks — the cockpit derives it from the next task automatically, so setting one would just freeze a stale copy. ONLY when the focused project has NO open tasks, review its recentlyCompleted tasks and set a next-action with projects.setNextAction that STARTS WITH exactly '[Advise] ' and proposes the path forward (e.g. '[Advise] The v1 milestone shipped — consider starting the billing integration next'). No id.",
